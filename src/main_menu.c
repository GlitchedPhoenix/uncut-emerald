...
static void Task_WaitForNewGamePlus(u8);
...
static void Task_ExplainNewGamePlus(u8);
...
enum
{
	...
	HAS_NGPLUS, //CONTINUE, NEW GAME, NEW GAME+, OPTION
	HAS_NGPLUS_MYSTERY_GIFT, //CONTINUE, NEW GAME, NEW GAME+, MYSTERY GIFT, OPTION
	HAS_NGPLUS_MYSTERY_EVENTS, //CONTINUE, NEW GAME, NEW GAME+, MYSTERY GIFT, MYSTERY EVENTS, OPTION
};
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (gSaveFileStatus)
        {
            case SAVE_STATUS_OK:
				...
				if (FlagGet(FLAG_SYS_NGPLUS_ENABLE))
                    tMenuType += 3;
        ...
        if (sCurrItemAndOptionMenuCheck & OPTION_MENU_FLAG)   // are we returning from the options menu?
        {
            switch (tMenuType)  // if so, highlight the OPTIONS item
            {
				...
				case HAS_NGPLUS:
					sCurrItemAndOptionMenuCheck = 3;
                    break;
				case HAS_NGPLUS_MYSTERY_GIFT:
					sCurrItemAndOptionMenuCheck = 4;
                    break;
				case HAS_NGPLUS_MYSTERY_EVENTS:
...
static void Task_MainMenuCheckBattery(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        {
			if (!(FlagGet(FLAG_NGP_EXPLAINED)) & (FlagGet(FLAG_SYS_NGPLUS_ENABLE)))
				gTasks[taskId].func = Task_ExplainNewGamePlus;
			else
...
static void Task_WaitForNewGamePlus(u8 taskId)
{
    RunTextPrinters();
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (gTasks[taskId].tMenuType)
        {
			...
			case HAS_NGPLUS:
                ...
                AddTextPrinterParameterized3(2, FONT_NORMAL, 0, 1, sTextColor_Headers, TEXT_SKIP_DRAW, gText_MainMenuContinue);
                AddTextPrinterParameterized3(3, FONT_NORMAL, 0, 1, sTextColor_Headers, TEXT_SKIP_DRAW, gText_MainMenuNewGame);
				AddTextPrinterParameterized3(4, FONT_NORMAL, 0, 1, sTextColor_Headers, TEXT_SKIP_DRAW, gText_MainMenuNewGamePlus);
                AddTextPrinterParameterized3(5, FONT_NORMAL, 0, 1, sTextColor_Headers, TEXT_SKIP_DRAW, gText_MainMenuOption);
                MainMenu_FormatSavegameText();
                PutWindowTilemap(2);
                PutWindowTilemap(3);
...
static bool8 HandleMainMenuInput(u8 taskId)
{
    ...
    {
        if ((tMenuType == HAS_MYSTERY_EVENTS || tMenuType == HAS_NGPLUS_MYSTERY_GIFT || tMenuType == HAS_NGPLUS_MYSTERY_EVENTS) && tIsScrolled == TRUE && tCurrItem == 1)
        {
    ...
    {
        if ((tMenuType == HAS_MYSTERY_EVENTS || tMenuType == HAS_NGPLUS_MYSTERY_GIFT || tMenuType == HAS_NGPLUS_MYSTERY_EVENTS) && tCurrItem == 3 && tIsScrolled == FALSE)
        {
            ChangeBgY(0, 0x2000, BG_COORD_ADD);
            ChangeBgY(1, 0x2000, BG_COORD_ADD);
...
static void Task_ExplainNewGamePlus(u8 taskId)
{
	if (!gPaletteFade.active)
    {
		CreateMainMenuErrorWindow(gText_NGPlus);
		gTasks[taskId].func = Task_WaitForNewGamePlus;
	}
}
...
static void Task_HandleMainMenuAPressed(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        if (gTasks[taskId].tMenuType == HAS_MYSTERY_EVENTS || gTasks[taskId].tMenuType == HAS_NGPLUS_MYSTERY_GIFT || gTasks[taskId].tMenuType == HAS_NGPLUS_MYSTERY_EVENTS)
        ...
        switch (action)
        {
            ...
            default:
				FlagClear(FLAG_NEW_GAME_PLUS_ACTIVE);
                gPlttBufferUnfaded[0] = RGB_BLACK;
                gPlttBufferFaded[0] = RGB_BLACK;
                gTasks[taskId].func = Task_NewGameBirchSpeech_Init;
                break;
			case ACTION_NGPLUS:
				FlagSet(FLAG_NEW_GAME_PLUS_ACTIVE);
				gPlttBufferUnfaded[0] = RGB_BLACK;
                gPlttBufferFaded[0] = RGB_BLACK;
                gTasks[taskId].func = Task_NewGameBirchSpeech_Init;
                break;
		...
		case HAS_NGPLUS_MYSTERY_EVENTS:
            switch (selectedMenuItem)
            {
                ...
                case 4:
                    ...
                    break;
				case 5:
                    SetGpuReg(REG_OFFSET_WIN0V, MENU_WIN_VCOORDS(7) - MENU_SCROLL_SHIFT);
                    break;
            }
            break;
    }
